...
# === 模型加载（GPU/4bit） ===
# 与 Chat_RAG_vecDB.py 保持一致，启用 GPU 和 4bit 量化
if gr.NO_RELOAD:
    BASE_DIR = r"C:\\Users\\c1052689\\hug_models\\Qwen2.5-0.5B-Instruct"
    tok = AutoTokenizer.from_pretrained(
        BASE_DIR, use_fast=False, local_files_only=True
    )
    ...
    pipe = pipeline(
        "text-generation", model=model, tokenizer=tok, max_new_tokens=512
    )
...
BASE_MSG_DIR = Path("./msgs/msgs_RAG")
...
def render_qwen_trim(
    ...
    if n_ctx is None:
        # Try tokenizer's max length
        ctx = None
        try:
            if 'tok' in globals() and tok is not None:
                ctx = getattr(tok, "model_max_length", None)
                if not isinstance(ctx, int) or ctx <= 0 or ctx > 10**8:
                    ctx = None
        except Exception:
            ctx = None
        # Then try model.config common fields
        if ctx is None:
            try:
                ...
                if cfg is not None:
                    ...
                    if not isinstance(ctx, int) or ctx <= 0:
                        ctx = getattr(cfg, "max_sequence_length", None)
            except Exception:
                ctx = None
        # Safe default
        n_ctx = int(ctx) if isinstance(ctx, int) and ctx > 0 else 4096
...
def on_send(user_text: str,
    ...
    # Use HF pipeline for generation (GPU/4bit); ensures proper tokenization
    gen_out = pipe(
        prompt,
    ...
    reply = gen_out[0].get('generated_text', '').strip()
